...
export const searchEngines = [
  {
    ...
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-google" viewBox="0 0 16 16">
...
export function handleSearch(e) {
  if (e.key === "Enter" || e.type === "click") {
    ...
    const engine =
      searchEngines.find((s) => s.name === state.settings?.searchEngine) ||
      searchEngines[0];
...
export function autoSaveSettings(updates = null) {
  ...
  if (updates && typeof updates === "object") {
    ...
    if (themeVal) newSettings.theme = themeVal;

    const clockVal = getCustomSelectValue("clockStyleSelect");
    if (clockVal) newSettings.clockStyle = clockVal;

    const userNameInput = document.getElementById("userNameInput");
    if (userNameInput) newSettings.userName = userNameInput.value.trim();

    const showSecondsToggle = document.getElementById("showSecondsToggle");
    if (showSecondsToggle)
      newSettings.showSeconds = !!showSecondsToggle.checked;

    const radios = document.getElementsByName("clockFormat");
    if (radios && radios.length > 0) {
      for (let r of radios) {
        if (r.checked) newSettings.clockFormat = r.value;
      }
    }
